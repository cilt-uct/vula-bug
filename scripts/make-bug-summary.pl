...
my $bugs_60days = query_table($dbh, "select TOOL, COUNT(TOOL), count(DISTINCT CAUSED_AT), count(DISTINCT EID), ROUND((COUNT(TOOL)+COUNT(DISTINCT EID))/COUNT(DISTINCT CAUSED_AT),1) FROM SAKAI_BUGS WHERE COMMENT IS NULL and (UNIX_TIMESTAMP(now()) - UNIX_TIMESTAMP(BUG_DATE)) < 86400*60 GROUP BY TOOL HAVING TOOL IS NOT NULL ORDER BY COUNT(TOOL) DESC", $scripturi, 60, @headings);
